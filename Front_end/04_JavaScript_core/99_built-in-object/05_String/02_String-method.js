...
// String 객체의 모든 메서드는 String 래퍼(wrapper) 객체를 직접 변경할 수 없고,
...
console.log(str.includes('b'));     // false 반환
...
// 인수로 전달받은 정규 표현식과 매치하는 문자열을 검색하여 일치하는 문자열이 인덱스를 반환
...
console.log(str.substring(1,4));    // 두 번째 인덱스 위치 바로 이전 문자까지
console.log(str.substring(1));      // 두 번째 인수 생략 시 문자열 끝까지
...
console.log(str.replace('a', 'b'));         // 검색된 문자열이 여럿 존재할 경우 첫 번째로 검색된 문자열만 치환
console.log(str.replace(/j/i,'Z'));         // 첫 번째 인수로 정규표현식 전달
...
// 첫 번째 인수로 전달한 문자열 또는 정규식을 검색하여 문자열을 구분한 후 분리된 각 문자열로 이루어진 배열 반환
